...
public class ControllerServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		}else if("/ageGraph".equals(path)) {
			processAgeGraph(request,response);
		}else if("/edit".equals(path)) {
			processEdit(request,response);
		}else if("/updateVac".equals(path)) {
			processUpdateVac(request,response);
		}

	}

	private void processUpdateVac(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			student.setDtap6(dtap6);
			student.setDtap15(dtap15);
			student.setPolio6(polio6);
			student.setPolio15(polio15);
			...
			//轉發到編輯疫苗的頁面
			response.sendRedirect("immuStudentList.do?teacherId=0");
		} catch (Exception e) {
	...
	private void processAgeGraph(HttpServletRequest request, HttpServletResponse response) throws IOException {
		// 檢查session 沒有session重定向到login
				HttpSession session = request.getSession();
				Object obj = session.getAttribute("teacherId");
				if (obj == null) {
				...
				try {
					...
					int[] ageCount = Student.getAgeCount();
					request.setAttribute("ageCount", ageCount);

					RequestDispatcher rd = request.getRequestDispatcher("AgeGraph.jsp");
					rd.forward(request, response);
				} catch (Exception e) {
					out.println("We encounter some problems. Please Try Agian!");
					e.printStackTrace();
	...
	private void processIndex(HttpServletRequest request, HttpServletResponse response) throws IOException {
		// 檢查session 沒有session重定向到login
				HttpSession session = request.getSession();
				Object obj = session.getAttribute("teacherId");
				if (obj == null) {
	...
	private void processSortAge(HttpServletRequest request, HttpServletResponse response) throws IOException {
				...
				try {
					...
					RequestDispatcher rd = request.getRequestDispatcher("NewStudentList.jsp");
					rd.forward(request, response);
	...
	private void processImmuStudentNotYetList(HttpServletRequest request, HttpServletResponse response) throws IOException {
		// 檢查session 沒有session重定向到login
				...
				try {
					...
					RequestDispatcher rd = request.getRequestDispatcher("NewImmuStudentList.jsp");
					rd.forward(request, response);
	...
	private void processTeacherList(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			if (teacherId == 0) {
				request.setAttribute("teachers", teachers);      
				RequestDispatcher rd = request.getRequestDispatcher("NewTeacherList.jsp");
				rd.forward(request, response);
			} else {
				request.setAttribute("teachers", teachers);
				RequestDispatcher rd = request.getRequestDispatcher("NewTeacherList.jsp");
