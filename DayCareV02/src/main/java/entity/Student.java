...
public class Student extends Person{
	...
	//計算不同age的學生數量
	public static int[] getAgeCount() throws Exception {	
		StudentDAO dao = new StudentDAO();
		List<Student> students = dao.findAll();
		int[] age = new int[6];
		age[0] = (int) students.stream().filter(x -> x.getAge()<=12 && x.getAge()>=6).count();
		age[1] = (int) students.stream().filter(x -> x.getAge()<=24 && x.getAge()>=13).count();
		age[2] = (int) students.stream().filter(x -> x.getAge()<=35 && x.getAge()>=25).count();
		age[3] = (int) students.stream().filter(x -> x.getAge()<=47 && x.getAge()>=36).count();
		age[4] = (int) students.stream().filter(x -> x.getAge()<=59 && x.getAge()>=48).count();
		age[5] = (int) students.stream().filter(x -> x.getAge()<=65 && x.getAge()>=60).count();
